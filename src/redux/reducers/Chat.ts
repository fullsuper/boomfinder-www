...
const users:Array<any> = [];// from "../app/chatPanel/data/chatUsers";
const conversationList:Array<any> = []; // from "../app/chatPanel/data/conversationList";
import {
...
export default (state: any = INIT_STATE, action: AnyAction) => {
  switch (action.type) {
    case FILTER_CONTACT: {
      if (action.payload === "") {
        return {
    ...
    case SUBMIT_COMMENT: {
      ...
      return {
        ...
        message: "",
        conversationList: state.conversationList.map((conversationData: any) => {
    ...
    case FETCH_ALL_CHAT_USER_SUCCESS: {
      return {
        ...state,
        contactList: action.payload.filter((user: any) => !user.recent),
        chatUsers: action.payload.filter((user: any) => user.recent),
        loader: false
      };
    }
